...
function view_all($conn){
    ...
    ';
    $result = $conn->query($query_patient);
    // Check if the query was successful
    if (!$result) {
        die("Error in SQL query: " . $conn->error);
    }
    ...
    WHERE
        patient.patient_number = '.$_POST["patient_id"].';
    ';
    $result = $conn->query($query_treatment);
    // Check if the query was successful
    if (!$result) {
        die("Error in SQL query: " . $conn->error);
    }
